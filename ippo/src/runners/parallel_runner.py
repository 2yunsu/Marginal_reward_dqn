...
class ParallelRunner:
    ...
    def run(self, test_mode=False):
        ...
        # print("-------------------infos: ", infos)
        # cur_stats.update({k: sum(d.get(k, 0) for d in infos) for k in set.union(*[set(d) for d in infos])}) #default
        # -------------------yslee1:ia2c
        # cur_stats.update({ 
        #     k: sum(
        #         v if isinstance(v, (int, float)) else sum(v) if isinstance(v, list) else 0
        #         for d in infos for v in ([d.get(k)] if not isinstance(d.get(k), list) else d.get(k))
        #     )
        ...
        for d in final_env_infos:
            for k, v in d.items():
                # Predator-Prey 환경에서는 prey_alive와 관련된 통계를 추가로 관리
                if k == "prey_alive":
                    if k not in cur_stats:
                        cur_stats[k] = []
                    cur_stats[k].append(sum(v))  # 살아있는 먹잇감의 수를 저장
                else:
                    if k not in cur_stats:
                        cur_stats[k] = []
                    cur_stats[k].append(v)
        # -------------------yslee2:ippo
        # cur_stats["n_episodes"] = self.batch_size + cur_stats.get("n_episodes", 0) #default
        cur_stats["n_episodes"] = cur_stats.get("n_episodes", 0) + self.batch_size #yslee

        # 에피소드 길이 통합
        # cur_stats["ep_length"] = cur_stats.get("ep_length", 0) + episode_lengths #default
        cur_stats["ep_length"] = cur_stats.get("ep_length", 0) + sum(episode_lengths) #yslee
    ...
    def _log(self, returns, stats, prefix):
        ...
        for k, v in stats.items():
            if k != "n_episodes":
                # self.logger.log_stat(prefix + k + "_mean" , v/stats["n_episodes"], self.t_env) #default
                # -------------------yslee
                if stats.get("n_episodes", 0) == 0:  # n_episodes가 0이거나 없을 경우
                    self.logger.log_stat(prefix + k + "_mean", 0, self.t_env)  # 기본값으로 0 기록
                else:
                    if isinstance(v, list):  # `v`가 리스트인 경우
                        v_mean = sum(v) / len(v) if v else 0
                    else:  # `v`가 숫자인 경우
                        v_mean = v

                self.logger.log_stat(prefix + k + "_mean", v_mean / stats["n_episodes"], self.t_env)
                # -------------------yslee
